...
def get_vals_resp_vs_param(data_s, p, repetition_keys = ['repeat'], means='ROIs', param=None, param_type=None, subset_rois = None):
    ...
    for data in data_s:
        ep = EpisodeData(data, 
                         protocol_name=p, 
                         quantities=['dFoF'])
        resp_cond, pos_cond, neg_cond = calc_responsiveness2(ep, 
                                                             data.nROIs, 
                                                             alpha = 0.05, 
                                                             t_window = 1.5, 
                                                             repetition_keys = repetition_keys, 
    ...
    if means == 'ROI':
        #average all ROIs 
        #size (total ROIs x 8 (ex : contrasts))
        resp_cond_s = np.concatenate(resp_cond_s, axis=0) 
        ...
        ns_col_sums   = np.sum(ns_cond_s, axis=0)

        final_resp_ = (resp_col_sums/nROIs)*100
        final_pos_  = (pos_col_sums/nROIs)*100
        final_neg_  = (neg_col_sums/nROIs)*100
        final_ns_   = (ns_col_sums/nROIs)*100
...
def plot_resp_vs_param(data_s, 
    ...
    '''
    if param==None: #is this correct?
        pos_1 = pos[::2]   # even indices: 0, 2, 4, ...
        pos_2 = pos[1::2]  # odd indices: 1, 3, 5, ...
        neg_1 = neg[::2]
        neg_2 = neg[1::2]
        ns_1 = ns[::2]
        ns_2 = ns[1::2]

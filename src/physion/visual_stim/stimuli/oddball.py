...
class stim(visual_stim):
    ...
    def __init__(self, protocol):
        ...
        print(NperRepeat, NperRepeat*protocol['N_repeat'])
        protocol['N-repeat'] = NperRepeat*protocol['N_repeat']
        ...
        for repeat in range(protocol['N_repeat']):
            ...
            print("first redundants", repeat*NperRepeat,repeat*NperRepeat+NmSR+iShifts[repeat]-1)
            print("deviant", repeat*NperRepeat+NmSR+iShifts[repeat])
            print("last redundants", repeat*NperRepeat+NmSR+iShifts[repeat]+1, (repeat+1)*NperRepeat)

            for i in range(repeat*NperRepeat,
                           repeat*NperRepeat+NmSR+iShifts[repeat]-1):
                print("i", i)
                self.experiment['angle'][i] = params['angle-redundant']
                print("angle : ", params['angle-redundant'])

            # deviant at random iShift
            self.experiment['angle'][repeat*NperRepeat+NmSR+iShifts[repeat]-1] = params['angle-deviant']
            print("i", repeat*NperRepeat+NmSR+iShifts[repeat]-1)
            print("angle : ", params['angle-deviant'])

            # last redundants from random iShift
            
            for i in range(repeat*NperRepeat+NmSR+iShifts[repeat], (repeat+1)*NperRepeat):
                print("i", i)
                self.experiment['angle'][i]  = params['angle-redundant']
                print("angle : ", params['angle-redundant'])
                
        
        print(self.experiment['angle'])
    ...
    def get_image(self, index,
        ...
        """
        print("index:", index, "angle:", self.experiment['angle'][index])
